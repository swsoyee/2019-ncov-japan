...
clusterData <- reactive({
  ...
  if (length(prefCode) > 0) {
    linkFilter <-
      GLOBAL_VALUE$signateLink[`id1-1` %in% prefCode |
        `id2-1` %in% prefCode]
    idFilter <- unique(c(linkFilter$罹患者id1, linkFilter$罹患者id2))
    signateDetailFilter <-
      GLOBAL_VALUE$signateDetail[罹患者id %in% idFilter |
        都道府県コード %in% prefCode]
...
output$clusterDateRangeSelector <- renderUI({
  ...
  if (!is.null(node) && nrow(node) > 0) {
    ...
    tagList(tags$b(i18n$t(
      "該当地域には感染者が確認されていません。またはデータの更新が必要です。"
    )))
  }
})
...
output$clusterProfileSearchBox <- renderUI({
  ...
  if (!is.null(node) && nrow(clusterData()$node) > 0) {
    choicesLabel <-
      paste0(node$受診都道府県, node$都道府県別罹患者No, "（", node$年代, node$性別, "）")
    choices <- node$罹患者id
...
output$profile <- renderUI({
  if (!is.null(input$searchProfileInCluster) &&
    !is.null(clusterData()$node) && nrow(clusterData()$node) > 0) {
    # 検索ボックスで検索する場合
    patientInfo <-
      clusterData()$node[罹患者id == input$searchProfileInCluster]
    ...
    if (length(patientInfo$label) > 0) {
      ...
      confirmedDate <-
        ifelse(profile[2] != "", profile[2], i18n$t("不明"))
      ...
      # 行動歴
      activityLog <-
        ifelse(profile[7] == "", i18n$t("不明"), profile[7])
      # ステータス
      statusBadge <- ""
      if (profile[9] == "罹患中") {
...
output$clusterNetwork <- renderEcharts4r({
  ...
  if (!is.null(node)) {
    ...
    function(params) {
      if (params.value) {
        ...
        const minDate = Date.parse("',
            input$clusterDateRange[1],
            '")
        const maxDate = Date.parse("',
            input$clusterDateRange[2],
            '")
  ...
  "
      ...
      e_title(
        ...
        subtext = paste0(
          i18n$t("公表日："),
          min(as.Date(node$公表日), na.rm = T),
          " ~ ",
          max(as.Date(node$公表日), na.rm = T)
        )
      )
  } else {
    return()
