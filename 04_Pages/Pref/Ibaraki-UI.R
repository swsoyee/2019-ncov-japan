  ...
  fluidRow(
    column(
      ...
      userBox(
        ...
        sprintf(
          i18n$t(
            "こちらは%sの発生状況をまとめたページです。厚労省のまとめより早く状況を把握できますが、県の集計時間は厚労省の発表時間と完全に一致していないため、タイムラグによる数値の違いが生じる可能性もありますので、予めご注意ください。また、速報では陰性から再び陽性になった人は再計算に含めていないため、自治体発表の陽性者数と数値が異なる場合があります。"
          ),
          i18n$t("茨城県")
        ),
        footer = tagList(tags$small(
          ...
          tags$a(
            ...
            i18n$t("茨城県（非公式）新型コロナウイルス感染症対策サイト")
          )
        ))
      )
    ),
    column(
      width = 7,
      uiOutput("IbarakiValueBoxes") %>% withSpinner(proxy.height = "200px")
    )
  ),
  fluidRow(
    box(
      width = 12,
      closable = F,
      title = tagList(icon("chart-line"), sprintf(i18n$t("%sの発生状況"), i18n$t("茨城県"))),
      fluidRow(
        column(
          width = 6,
          echarts4rOutput("IbarakiContact") %>% withSpinner()
        ),
        column(
          width = 6,
          echarts4rOutput("IbarakiSummary") %>% withSpinner()
        )
      ),
