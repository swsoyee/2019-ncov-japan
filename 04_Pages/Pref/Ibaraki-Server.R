    ...
    tagList(
      fluidRow(
        ...
        createValueBox(value = totalPositive,
                       subValue = paste0(i18n$t('速報：'), sum(byDate[, 9, with = T], na.rm = T)), 
...
output$IbarakiSummary <- renderEcharts4r({
  ...
  data %>%
    e_chart(date) %>%
    e_bar(検査数, name = '検査実施人数', y_index = 1, barGap = '-100%', color = middleYellow) %>%
    e_bar(陽性数, name = '陽性者数', y_index = 1, color = middleRed) %>%
    e_line(検査数累計, name = '検査実施人数累計', color = darkYellow) %>%
    e_line(陽性数累計, name = '陽性者数累計', color = darkRed) %>%
